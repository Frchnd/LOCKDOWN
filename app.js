...
function joinMaya(){state.survivors.push({id:'maya',name:'Maya',role:'Mantan Perawat',skill:'Medis',personality:'Tenang',trust:45,lastTalkDay:0,lastHelpDay:0,joinedDay:state.day,sideQuestDone:false});state.morale=clamp(state.morale+5);log('Maya bergabung.');closeModal()}
function joinRaka(){state.survivors.push({id:'raka',name:'Raka',role:'Mantan Teknisi Listrik',skill:'Teknisi',personality:'Pragmatis',trust:40,lastTalkDay:0,lastHelpDay:0,joinedDay:state.day,sideQuestDone:false});state.morale=clamp(state.morale+3);if(survivor('maya'))state.relations.tension=clamp(state.relations.tension+20);log('Raka bergabung.');closeModal()}
...
function dashboard(){
 ...
 <div class="version">LOCKDOWN · PWA v0.16 · SURVIVOR & QUEST SYSTEM</div>
 </main>`;
}
...
function panel(){
 ...
 const panelClass=p==='radio'?'radio-panel':p==='medis'?'medical-panel':p==='gudang'?'warehouse-panel':p==='kasur'?'sleep-panel':p==='expedition'?'expedition-panel':p==='survivor'?'survivor-panel':p==='questlog'?'questlog-panel':'';
 const headClass=p==='radio'?'radio-head':p==='medis'?'medical-head':p==='gudang'?'warehouse-head':p==='kasur'?'sleep-head':p==='expedition'?'expedition-head':p==='survivor'?'survivor-head':p==='questlog'?'questlog-head':'';
 const subtitle=p==='radio'?'COMMUNICATION STATION // OFFLINE ARCHIVE':p==='medis'?'MEDICAL STATION // HEALTH & RADIATION':p==='gudang'?'SUPPLY STORAGE // FOOD & DRINK':p==='kasur'?'REST MODULE // SLEEP & RECOVERY':p==='expedition'?'SURFACE OPERATIONS // RISK & LOOT':p==='survivor'?'SURVIVOR RELATION // TRUST & UPKEEP':p==='questlog'?'OBJECTIVE CONTROL // MAIN · SIDE · COMPLETED':'';
