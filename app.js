...
function dashboard(){
 ...
 <div class="version">LOCKDOWN · PWA v0.19.1 · FINAL UI/UX POLISH</div>
 </main>`;
}
...
function panel(){
 ...
 if(p==='workbench'){
  ...
  const tab=activePanelTab('workbench','craft');
  ...
  ${panelTabs('workbench',[['craft','CRAFTING'],['upgrades','UPGRADES']])}
  <div class="tab-content">${tab==='upgrades'?upgradeContent:craftContent}</div>`;
 }
 if(p==='radio'){
  title='Radio';
 ...
 if(p==='archive'){
  ...
  ${panelTabs('archive',[['timeline','TIMELINE',journal.length],['radio','RADIO',radio.length],['surface','SURFACE',visited.length]])}
  <div class="tab-content archive-tab-content">${tab==='radio'?radioHtml:tab==='surface'?surfaceHtml:timelineHtml}</div>
