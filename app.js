...
let state=null, view={screen:'menu',panel:null,panelOrigin:'game',dashboardScroll:0,tabs:{workbench:'craft',expedition:'available',questlog:'main',archive:'timeline'},modal:null,prologueFrame:0,expeditionResult:null,expeditionRun:null};
const scrollSoon=(y=0)=>requestAnimationFrame(()=>requestAnimationFrame(()=>window.scrollTo({top:Math.max(0,Number(y)||0),left:0,behavior:'auto'})));
const activePanelTab=(panel,fallback)=>view.tabs?.[panel]||fallback;
function panelTabs(panel,items){const active=activePanelTab(panel,items[0][0]);return `<nav class="panel-tabs" aria-label="Navigasi ${panel}">${items.map(([id,label,count])=>`<button class="panel-tab ${active===id?'active':''}" data-panel-tab="${panel}:${id}"><span>${label}</span>${count!==undefined?`<b>${count}</b>`:''}</button>`).join('')}</nav>`}
const STATION_ICON={
 radio:'<path d="M12 20V10m0 0 5-6m-5 6L7 4M8 20h8M5.5 8.5a9 9 0 0 0 0 7m13-7a9 9 0 0 1 0 7M8.5 11a5 5 0 0 0 0 3m7-3a5 5 0 0 1 0 3"/>',
 medis:'<path d="M9 4h6v5h5v6h-5v5H9v-5H4V9h5z"/>',
 gudang:'<path d="M4 8l8-4 8 4v10l-8 3-8-3zM4 8l8 4 8-4M12 12v9M8 10l8-4"/>',
 kasur:'<path d="M4 18V9m0 6h16v3M7 13h5a3 3 0 0 0-3-3H7zm5 0h8v-2a3 3 0 0 0-3-3h-5"/>',
 security:'<path d="M12 3l7 3v5c0 4.7-2.8 8-7 10-4.2-2-7-5.3-7-10V6zm-3 9 2 2 4-5"/>',
 generator:'<path d="M13 2 6 13h5l-1 9 8-12h-5z"/>',
 workbench:'<path d="m5 19 6-6m2-2 6-6M8 5l3 3-3 3-3-3zm8 8 3 3-3 3-3-3z"/>',
 expedition:'<path d="M12 3a9 9 0 1 0 9 9 9 9 0 0 0-9-9zm3.5 5.5-2 5-5 2 2-5z"/>'
};
function stationIcon(id,extra=''){const path=STATION_ICON[id]||'<circle cx="12" cy="12" r="5"/>';return `<span class="station-emblem ${extra}" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">${path}</svg></span>`}
...
function restartGame(){const set=state.settings;localStorage.removeItem(SAVE);state=fresh();state.settings=set;localStorage.setItem(SETTINGS,JSON.stringify(set));localStorage.setItem(SAVE,JSON.stringify(state));view.modal=null;view.screen='prologue';view.panel=null;view.panelOrigin='game';view.prologueFrame=0;render();scrollSoon(0)}
...
function openPanel(p,origin='game'){if(view.screen==='game'&&!view.panel&&origin==='game')view.dashboardScroll=window.scrollY||0;view.screen='game';view.panel=p;view.panelOrigin=origin;if(['gudang','generator','radio'].includes(p))completeOnboarding(p);if(p==='radio')AudioUI.radioBurst();render();scrollSoon(0)}
function back(){if(view.modal){if(view.modal.kind==='gameover'){toast('Game Over: pilih Restart atau Main Menu.');return}closeModal();return}if(view.panel){if(view.panel==='radio')AudioUI.stopRadio();const origin=view.panelOrigin||'game';if(String(origin).startsWith('panel:')){view.panel=String(origin).slice(6);view.panelOrigin='game';view.screen='game';render();scrollSoon(0)}else{view.panel=null;view.panelOrigin='game';if(origin==='menu'){view.screen='menu';render();scrollSoon(0)}else{view.screen='game';render();scrollSoon(view.dashboardScroll||0)}}return}view.screen='menu';render();scrollSoon(0)}
...
function finishExpeditionRun(id){
 ...
 view.expeditionRun=null;save();view.panel='expedition';render();scrollSoon(0);
}
...
const PANEL_VISUALS={
 radio:{src:'assets/visual/radio.webp',alt:'Ruang komunikasi bunker'},
 medis:{src:'assets/visual/medis.webp',alt:'Ruang medis bunker'},
 gudang:{src:'assets/visual/gudang.webp',alt:'Gudang persediaan bunker'},
 security:{src:'assets/visual/security.webp',alt:'Ruang kontrol keamanan bunker'},
 generator:{src:'assets/visual/generator.webp',alt:'Ruang generator bunker'},
 workbench:{src:'assets/visual/workbench.webp',alt:'Ruang kerja dan crafting bunker'},
 expedition:{src:'assets/visual/expedition.webp',alt:'Permukaan kota pascabencana'}
};
function survivorPortrait(id){return id==='maya'?'assets/visual/maya.webp':id==='raka'?'assets/visual/raka.webp':''}
function avatarHtml(s,large=false){const src=survivorPortrait(s?.id);return src?`<div class="avatar portrait ${large?'large':''}"><img src="${src}" alt="${s.name}"></div>`:`<div class="avatar ${large?'large':''}">${s?.name?.[0]||'?'}</div>`}
function panelVisual(id){
 if(id==='survivor'){
  const s=survivor(view.survivorId);const src=survivorPortrait(s?.id);if(!src)return '';
  return `<section class="panel-visual npc-visual"><img src="${src}" alt="${s.name}"><div class="panel-visual-shade"></div><div class="panel-visual-caption"><span>SURVIVOR DOSSIER</span><b>${s.name}</b></div></section>`;
 }
 const v=PANEL_VISUALS[id];if(!v)return '';
 return `<section class="panel-visual visual-${id}"><img src="${v.src}" alt="${v.alt}"><div class="panel-visual-shade"></div></section>`;
}
...
function menu(){const saved=hasSave();const install=installPrompt&&!isStandalone()?'<button class="menu-util" data-act="install"><span>INSTALL PWA</span><small>OFFLINE</small></button>':'';return `<main class="screen menu"><div class="menu-atmosphere"></div><div class="menu-frame"><div class="menu-topline"><span>LOCKDOWN PROTOCOL</span><span class="signal">● SYSTEM READY</span></div><div class="menu-inner"><div class="brand-block"><div class="eyebrow">Narrative Survival Management</div><img class="menu-logo-img" src="assets/visual/lockdown-logo.webp" alt="LOCKDOWN"><div class="chapter-mark">CHAPTER 01 // BUNKER 7B</div><p class="tagline">Bunker bukan tujuan akhir.<br>Bunker hanya membeli waktu.</p></div>${saved?saveDossier():''}<div class="menu-actions"><button class="btn primary continue-btn" data-act="continue" ${saved?'':'disabled'}><span>CONTINUE</span><small>${saved?'LANJUTKAN ACTIVE SAVE':'ACTIVE SAVE TIDAK DITEMUKAN'}</small></button><button class="btn newgame-btn" data-act="new"><span>NEW GAME</span><small>MULAI HARI 1 · 07:00</small></button></div><div class="menu-utils"><button class="menu-util" data-act="archive"><span>STORY ARCHIVE</span><small>TRANSMISSION</small></button><button class="menu-util" data-act="settings"><span>SETTINGS</span><small>AUDIO · HAPTIC</small></button><button class="menu-util" data-act="credits"><span>CREDITS</span><small>BUILD INFO</small></button>${install}</div><div class="version">PWA v0.19.5 · LAYOUT & SURVIVOR POLISH</div></div></div></main>`}
function credits(){return `<main class="screen credits-screen"><header class="panel-head"><button class="back" data-act="back">‹</button><div><h2>CREDITS</h2><span>LOCKDOWN // CHAPTER 01</span></div></header><section class="credits-hero"><img class="credits-logo-img" src="assets/visual/lockdown-logo.webp" alt="LOCKDOWN"><p>Narrative Survival Management · Android portrait · Offline PWA</p></section><section class="card credits-card"><div class="story-tag">PROJECT</div><h3>LOCKDOWN</h3><p>Chapter 1 · Hari 1–7. Survival, bunker management, survivor relationship, radio narrative, dan expedition/scavenging.</p></section><section class="card credits-card"><div class="story-tag">BUILD</div><p>HTML + CSS + Vanilla JavaScript<br>Local save · Offline-first · No account · No telemetry gameplay</p></section><div class="version">PWA v0.19.5 · LAYOUT & SURVIVOR POLISH</div></main>`}
...
function dashboard(){
 const q=mainQuest();
 const stats=[['health','Kesehatan',state.health],['hunger','Lapar',state.hunger],['thirst','Haus',state.thirst],['fatigue','Kelelahan',state.fatigue],['morale','Moral',state.morale],['radiation','Radiasi',state.radiation],['power','Daya',state.power],['integrity','Bunker',state.integrity]];
 const stations=[['radio','Radio','Sinyal & arsip'],['medis','Medis','Obat & recovery'],['gudang','Gudang','Food & drinks'],['kasur','Kasur','Tidur & pulih'],['security','Keamanan','Scan & repair'],['generator','Generator','Fuel & daya'],['workbench','Meja Kerja','Craft & upgrade'],['expedition','Ekspedisi','Scavenge & story']];
 ...
 const pending=state.dailyObjectives.find(x=>!x.done);
 ...
 <header class="topbar dashboard-topbar">
  ...
  <button class="icon-btn system-btn" data-act="menu" aria-label="Buka Settings">⋮</button>
 </header>
 ...
 <div class="section-head compact-head"><h3>Status</h3><span>8 indikator</span></div>
 <section class="status-console">${stats.map(([k,n,v])=>`<div class="status-line ${statClass(k,v)}"><div class="status-label"><span>${n}</span><small>${statState(k,v)}</small></div><div class="status-mini-bar"><i style="width:${v}%"></i></div><b>${Math.round(v)}</b></div>`).join('')}</section>
 <div class="section-head compact-head"><h3>Station</h3><span>${state.onboarding<4?'Onboarding '+(state.onboarding+1)+'/4':'8 modul'}</span></div>
 <section class="station-grid dashboard-stations compact-stations">${stations.map(([id,n,sub])=>{const locked=stationLocked(id)||isGameOver();const reason=stationLockReason(id);const upgradeKey=id==='security'?'keamanan':id;const hasLv=['generator','medis','kasur','security','workbench'].includes(id);return `<button class="station station-${id} ${locked?'locked':''} ${id===activeOnboarding?'active-objective':''}" data-panel="${id}" ${locked?'disabled':''}><div class="station-top">${stationIcon(id)}${hasLv&&!stationLocked(id)?`<span class="badge">LV ${state.upgrades[upgradeKey]}</span>`:''}</div><span class="station-copy"><b>${n}</b><small>${sub}</small></span>${reason?`<span class="lock-reason">${reason}</span>`:''}</button>`}).join('')}</section>
 ${state.survivors.length?`<div class="section-head compact-head"><h3>Survivor</h3><span>${state.survivors.length}/4 · Tension ${Math.round(state.relations.tension)}</span></div><section class="card survivor-list survivor-dashboard compact-survivors">${state.survivors.map(s=>{const sq=survivorSideQuest(s);return `<button class="survivor" data-survivor="${s.id}">${avatarHtml(s)}<div class="grow survivor-copy"><b>${s.name}</b><small>${s.role}${sq&&!sq.done?` · ${sq.name}`:''}</small></div><div class="trust"><small>${trustLabelFor(s.trust).toUpperCase()}</small><b>${Math.round(s.trust)}</b></div></button>`}).join('')}<div class="survivor-roster-meta"><span>ROSTER CAPACITY</span><b>${Math.max(0,4-state.survivors.length)} SLOT TERSEDIA</b></div></section>`:''}
 <details class="dashboard-fold"><summary><div><span>DAILY OBJECTIVE</span><b>${done}/3${pending?' · '+pending.text:''}</b></div><div class="fold-dots">${state.dailyObjectives.map(o=>`<i class="${o.done?'done':''}"></i>`).join('')}</div></summary><div class="fold-body"><div class="daily-progress"><i style="width:${done/3*100}%"></i></div>${state.dailyObjectives.map(o=>`<div class="objective ${o.done?'done':''}"><span class="dot">${o.done?'✓':''}</span><span>${o.text}</span></div>`).join('')}<div class="daily-reward"><span>3/3 REWARD</span><b>+2 Komponen · +1 Filter</b></div></div></details>
 <details class="dashboard-fold activity-fold"><summary><div><span>ACTIVITY LOG</span><b>${latest?`D${latest.day} ${fmtHour(latest.hour)} · ${latest.text}`:'Belum ada aktivitas'}</b></div><span class="fold-count">${Math.min(4,state.logs.length)}</span></summary><div class="fold-body">${state.logs.length?state.logs.slice(0,4).map(l=>`<div class="log"><time>D${l.day} ${fmtHour(l.hour)}</time>${l.text}</div>`).join(''):'<div class="log">Belum ada aktivitas.</div>'}</div></details>
 <div class="version">LOCKDOWN · PWA v0.19.5 · LAYOUT & SURVIVOR POLISH</div>
 </main>`;
}
...
function panel(){
 ...
 if(p==='gudang'){
  ...
  <section class="warehouse-body-state"><div class="${statClass('hunger',state.hunger)}"><span>LAPAR</span><b>${Math.round(state.hunger)}</b><small>${statState('hunger',state.hunger)}</small></div><div class="${statClass('thirst',state.thirst)}"><span>HAUS</span><b>${Math.round(state.thirst)}</b><small>${statState('thirst',state.thirst)}</small></div></section>
 ...
 if(p==='medis'){
  ...
  ${maya?`<section class="medical-provider">${avatarHtml(maya)}<div class="grow"><span>MEDICAL SUPPORT</span><b>Maya · Mantan Perawat</b><p>Skill aktif di station: setiap Obat Darurat mendapat bonus +8 Health.</p></div><div class="medical-provider-trust"><small>TRUST</small><b>${Math.round(maya.trust)}</b></div></section>`:''}
 ...
 if(p==='workbench'){
  ...
  const craftContent=`<div class="section-head workbench-section-head"><h3>Fabrication</h3><span>0 jam · langsung selesai</span></div>
  ...
  const upgradeContent=`<div class="section-head workbench-section-head"><h3>Bunker Upgrades</h3><span>5 Komp → Lv2 · 9 Komp → Lv3</span></div><div class="upgrade-grid">${upgradeKeys.map(k=>upgradeCard(k,UPGRADE_LABELS[k])).join('')}</div>`;
 ...
 if(p==='radio'){
  ...
  const archiveHtml=archive.length?archive.slice(0,3).map((x,i)=>`<div class="radio-archive-row"><div class="radio-archive-meta"><span>${x.kind||'RX'}</span><time>D${x.day} ${fmtHour(x.hour)}</time></div><b>${x.label}</b><p>${x.text}</p></div>`).join(''):`<div class="radio-empty"><span>NO STORED SIGNAL</span><p>Belum ada hasil scan atau transmission yang tersimpan di Radio Archive.</p></div>`;
  ...
  <div class="section-head radio-section-head"><h3>Sinyal Terakhir</h3><span>${archive.length?Math.min(3,archive.length):0} terbaru</span></div>
 ...
 if(p==='expedition'){
  ...
  const locationRows=Object.entries(LOCS).map(([id,l])=>{
   ...
   const html=`<section class="exp-location ${unlocked?'':'locked'} ${story?'story':''}"><div class="exp-location-top"><div><span>${story?'STORY LOCATION':'SURFACE LOCATION'}${visited?' · VISITED':''}</span><b>${l.name}</b></div><span class="risk ${l.risk.toLowerCase()}">${l.risk}</span></div><p>${l.focus}</p><div class="exp-metrics"><div><small>WAKTU</small><b>${l.time} jam</b></div><div><small>BASE RAD</small><b>${l.rad}</b></div><div><small>EST. RAD</small><b>+${Math.round(r.total)}</b></div><div><small>FATIGUE</small><b>${Math.round(state.fatigue)}→${Math.round(p.after.fatigue)}</b></div></div><div class="costs"><span class="cost">World ${fmt(r.world)} Rad</span>${r.scout?'<span class="cost scout">Scout -2 Rad</span>':''}<span class="cost">Loot: ${l.focus}</span></div>${unlocked?`<button class="btn ${story?'primary':''}" data-exp="${id}" ${can?'':'disabled'}>${can?'RENCANAKAN EKSPEDISI':'KONDISI TIDAK LAYAK'}</button>`:`<button class="btn" disabled>TERKUNCI</button><div class="reason">${locationUnlockReason(id)}</div>`}</section>`;
   return {id,unlocked,story,html};
  });
  const tab=activePanelTab('expedition','available');
  const availableCount=locationRows.filter(x=>x.unlocked&&!x.story).length,storyCount=locationRows.filter(x=>x.story).length,lockedCount=locationRows.filter(x=>!x.unlocked&&!x.story).length;
  const shown=locationRows.filter(x=>tab==='story'?x.story:tab==='locked'?(!x.unlocked&&!x.story):(x.unlocked&&!x.story));
  const locations=shown.length?shown.map(x=>x.html).join(''):'<section class="questlog-empty">Tidak ada lokasi pada kategori ini.</section>';
  body=`${resultHtml}<section class="exp-command"><div><span>EXPEDITION CONTROL // DAY ${state.day}</span><strong>${ready.ok?'FIELD READY':'DEPARTURE LOCKED'}</strong><p>Ekspedisi memakai waktu bunker nyata. Lapar, Haus, Fatigue, Daya, dan survival damage terus berjalan selama perjalanan.</p></div><div class="exp-world"><small>WORLD</small><b>${wc.name}</b><span>Rad ${fmt(wc.rad)}${scouted?' · Scout -2':''}</span></div></section><section class="exp-readiness">${readiness}</section>${!ready.ok?'<div class="exp-warning"><b>Lanjut Jelajah nonaktif.</b><p>Semua syarat readiness harus lolos sebelum berangkat.</p></div>':''}<section class="exp-scout ${scouted?'active':''}"><div><span>SURFACE INTEL</span><b>${scouted?'SCOUT ACTIVE':'NO SCOUT DATA'}</b></div><p>${scouted?'Security scan aktif untuk Hari '+state.day+'. Estimasi Radiasi semua lokasi dikurangi 2.':'Scan di Keamanan sebelum berangkat untuk menurunkan estimasi Radiasi sebesar 2 hari ini.'}</p></section>${storyRows.length?`<div class="section-head exp-section-head"><h3>Mission Prioritas</h3><span>${storyRows.length} lokasi</span></div><div class="exp-location-list story-stack">${storyRows.map(x=>x.html).join('')}</div>`:''}<div class="section-head exp-section-head"><h3>Lokasi Tersedia</h3><span>${availableRows.length} lokasi</span></div><div class="exp-location-list">${availableRows.length?availableRows.map(x=>x.html).join(''):'<div class="exp-empty-note">Belum ada lokasi bebas yang tersedia.</div>'}</div><div class="section-head exp-section-head"><h3>Belum Terbuka</h3><span>${lockedRows.length} lokasi</span></div><section class="exp-locked-list">${compactLocked}</section>`;
 }
 if(p==='survivor'){
  title='Survivor';
  ...
  if(s){const q=survivorSideQuest(s),talked=s.lastTalkDay===state.day,helped=s.lastHelpDay===state.day,canHelp=s.trust>=50&&!helped;
   body=`<section class="survivor-command"><div class="survivor-ident">${avatarHtml(s,true)}<div><span>SURVIVOR // JOINED DAY ${s.joinedDay}</span><h2>${s.name}</h2><p>${s.role} · ${s.personality} · ${s.skill}</p></div></div><div class="trust-module"><div><span>TRUST</span><b>${Math.round(s.trust)}</b><em>${trustLabelFor(s.trust)}</em></div><div class="trust-bar"><i style="width:${s.trust}%"></i></div></div></section>
   <section class="relation-snapshot"><div><small>RELATION TENSION</small><b>${Math.round(state.relations.tension)} / 100</b><span>${tensionLabel()}</span></div><p>${state.survivors.length>1?'Trust, Moral, dan stok harian dapat menaikkan atau menurunkan ketegangan antar survivor.':'Ketegangan antar survivor mulai relevan setelah penghuni kedua bergabung.'}</p></section><section class="survivor-capacity-panel"><div><span>ROSTER</span><b>${state.survivors.length} / 4</b></div><div class="survivor-capacity-slots">${Array.from({length:4},(_,i)=>`<i class="${i<state.survivors.length?'filled':''}"></i>`).join('')}</div><small>${Math.max(0,4-state.survivors.length)} slot belum terisi</small></section>
 ...
 if(p==='questlog'){
  title='Quest Log';const qd=questLogData(),tab=activePanelTab('questlog','main');
  const mainHtml=`<section class="questlog-card main"><div class="questlog-meta"><span>ACTIVE</span><b>STAGE ${Math.min(qd.main.stage,10)}</b></div><h3>${qd.main.title}</h3><p>${qd.main.text}</p><div class="quest-progress"><i style="width:${Math.min(100,(Math.min(qd.main.stage,10)/10)*100)}%"></i></div></section>`;
  const sideHtml=qd.sides.length?`<section class="questlog-stack">${qd.sides.map(({survivor:s,quest:q})=>`<button class="questlog-card side" data-survivor="${s.id}"><div class="questlog-meta"><span>${s.name.toUpperCase()}</span><b>ACTIVE</b></div><h3>${q.name}</h3><p>${q.objective}</p><div class="questlog-reward"><span>REWARD</span><b>${q.reward}</b></div></button>`).join('')}</section>`:'<section class="questlog-empty">Belum ada Side Quest aktif.</section>';
  const completedHtml=qd.completed.length?`<section class="questlog-stack completed">${qd.completed.map(([type,name,text])=>`<div class="questlog-card completed"><div class="questlog-meta"><span>${type}</span><b>✓ SELESAI</b></div><h3>${name}</h3><p>${text}</p></div>`).join('')}</section>`:'<section class="questlog-empty">Belum ada quest yang selesai.</section>';
  body=`<section class="questlog-command"><div><span>CHAPTER 01 // QUEST CONTROL</span><strong>${state.story.chapterComplete?'CHAPTER COMPLETE':'OBJECTIVES ACTIVE'}</strong><p>Main Quest menggerakkan Chapter. Side Quest mendorong cerita survivor. Daily Objective tetap terpisah.</p></div><div class="questlog-count"><small>MAIN STAGE</small><b>${Math.min(qd.main.stage,10)} / 10</b></div></section><div class="section-head questlog-section-head"><h3>Main Quest</h3><span>1 aktif</span></div>${mainHtml}<div class="section-head questlog-section-head"><h3>Side Quest</h3><span>${qd.sides.length} aktif</span></div>${sideHtml}<details class="quest-completed-collapse"${qd.completed.length<=3?' open':''}><summary><span>Completed</span><b>${qd.completed.length}</b></summary>${completedHtml}</details>`;
 }
 if(p==='archive'){
  title='Story Archive';
  ...
  const tab=activePanelTab('archive','timeline');
  const timelineHtml=journal.length?`<section class="archive-timeline">${journal.map(x=>`<article class="archive-entry"><div class="archive-entry-meta"><span>${x.kind}</span><time>D${x.day} ${fmtHour(x.hour)}</time></div><h3>${x.title}</h3><p>${x.text}</p></article>`).join('')}</section>`:'<section class="questlog-empty">Belum ada story beat yang tercatat.</section>';
  const radioHtml=radio.length?`<section class="archive-radio">${radio.slice(0,12).map(x=>`<article><div><span>${x.kind||'RX'}</span><time>D${x.day} ${fmtHour(x.hour)}</time></div><b>${x.label}</b><p>${x.text}</p></article>`).join('')}</section>`:'<section class="questlog-empty">Belum ada transmission tersimpan.</section>';
  const surfaceHtml=`<section class="archive-visited">${visited.length?visited.map(x=>`<span>${x}</span>`).join(''):'<span>Belum ada lokasi dikunjungi.</span>'}</section>`;
  ...
  <div class="section-head archive-section-head"><h3>Timeline</h3><span>${journal.length} catatan</span></div>${timelineHtml}
  <div class="section-head archive-section-head"><h3>Radio Archive</h3><span>${radio.length} transmission</span></div>${radioHtml}
  <div class="section-head archive-section-head"><h3>Surface Record</h3><span>${visited.length} dikunjungi</span></div>${surfaceHtml}
  <button class="btn archive-replay" style="width:100%" data-act="replay-prologue">REPLAY PROLOGUE</button>`;
 }
 if(p==='settings'){
  title='Settings';
  body=settingsPanel();
 ...
 const subtitle=p==='radio'?'COMMUNICATION // SIGNAL & ARCHIVE':p==='medis'?'MEDICAL // HEALTH & RADIATION':p==='gudang'?'STORAGE // FOOD & DRINK':p==='kasur'?'REST // SLEEP & RECOVERY':p==='security'?'SECURITY // SCAN & STRUCTURE':p==='generator'?'POWER // FUEL & OUTPUT':p==='workbench'?'FABRICATION // CRAFT & UPGRADE':p==='expedition'?'SURFACE OPS // RISK & LOOT':p==='survivor'?'RELATION // TRUST & UPKEEP':p==='questlog'?'OBJECTIVES // MAIN · SIDE · DONE':p==='archive'?'STORY // TRANSMISSION & CHOICE':p==='settings'?'SYSTEM // AUDIO · HAPTIC · SAVE':'';
 const physical=['radio','medis','gudang','kasur','security','generator','workbench','expedition'].includes(p);return `<main class="screen panel ${panelClass}"><header class="panel-head ${headClass}"><button class="back" data-act="back">←</button>${physical?stationIcon(p,'panel-station-emblem'):''}<div class="grow"><h1>${title}</h1>${subtitle?`<span>${subtitle}</span>`:''}</div><small>D${state.day} ${fmtHour()}</small></header>${panelVisual(p)}${body}</main>`;
}
...
function bind(){
 ...
 document.querySelectorAll('[data-survivor]').forEach(b=>b.onclick=()=>{const origin=view.panel?`panel:${view.panel}`:'game';view.survivorId=b.dataset.survivor;openPanel('survivor',origin)});
 document.querySelectorAll('[data-panel-tab]').forEach(b=>b.onclick=()=>{const [panel,tab]=b.dataset.panelTab.split(':');view.tabs[panel]=tab;render();scrollSoon(0)});
...
function beginNewGame(){state=fresh();localStorage.setItem(SAVE,JSON.stringify(state));view.modal=null;view.screen='prologue';view.panel=null;view.panelOrigin='game';view.prologueFrame=0;render();scrollSoon(0)}
function confirmNewGame(){view.modal={kind:'confirm',tag:'SAVE PROTOCOL',title:'Mulai Game Baru?',text:'Active save akan diganti. Progress Chapter 1 yang sedang berjalan tidak bisa dikembalikan setelah New Game dimulai.',meta:'NEW GAME // DAY 1 · 07:00',choices:[['MULAI GAME BARU','Ganti active save dan mulai Prologue.',beginNewGame,'danger'],['BATAL','Pertahankan active save sekarang.',closeModal,'ghost']]};render()}
function confirmRestart(){view.modal={kind:'confirm',tag:'SYSTEM // RESTART',title:'Restart LOCKDOWN?',text:'Active game save akan dihapus dan permainan kembali ke Day 1 · 07:00. Pengaturan audio dan vibration tetap dipertahankan.',meta:'DESTRUCTIVE ACTION // SAVE RESET',choices:[['RESTART GAME','Hapus progress aktif dan mulai dari awal.',restartGame,'danger'],['BATAL','Kembali ke Settings tanpa perubahan.',closeModal,'ghost']]};render()}
function act(a){if(a==='install'){if(installPrompt){installPrompt.prompt();installPrompt.userChoice.finally(()=>{installPrompt=null;render()})}else toast('Gunakan menu browser → Install app');return}if(a==='continue'){state=load();view.screen='game';view.panel=null;view.panelOrigin='game';if(isGameOver())presentGameOver();else{render();scrollSoon(0)}}if(a==='new'){if(hasSave())confirmNewGame();else beginNewGame()}if(a==='archive'){state=load()||fresh();openPanel('archive','menu')}if(a==='settings'){state=load()||fresh();openPanel('settings','menu')}if(a==='credits'){view.screen='credits';view.panel=null;view.panelOrigin='menu';render();scrollSoon(0)}if(a==='prologue-next'){if(view.prologueFrame<6){view.prologueFrame++;render();scrollSoon(0)}else{state.story.prologueDone=true;log('LOCKDOWN engaged. Day 1 07:00.');save();view.screen='game';view.panel=null;render();scrollSoon(0)}}if(a==='skip'){if(!isGameOver())advance(1)}if(a==='menu'){openPanel('settings','game')}if(a==='back')back();if(a==='fuel')fuel();if(a==='med')useMed();if(a==='scan')scan();if(a==='repair')repair();if(a==='sleep'){const h=Number($('#sleepRange')?.value||4);sleep(h)}if(a==='radio-story')radioStory();if(a==='radio-random')radioRandom();if(a==='exp-result-close'){view.expeditionResult=null;if(!postTimeActionEvents())render();}if(a==='replay-prologue'){view.screen='prologue';view.panel=null;view.panelOrigin='game';view.prologueFrame=0;render();scrollSoon(0)}if(a==='to-menu'){view.screen='menu';view.panel=null;view.panelOrigin='game';save();render();scrollSoon(0)}if(a==='restart')confirmRestart()}
