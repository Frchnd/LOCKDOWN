...
const PANEL_VISUALS={
 ...
 workbench:{src:'assets/visual/workbench.webp',alt:'Meja kerja teknisi bunker'},
...
function dashboard(){
 ...
 <div class="section-head compact-head"><h3>Status</h3><span>${condition} · ${world.name}</span></div>
 ...
 <div class="section-head compact-head"><h3>Station</h3><span>${state.onboarding<4?'Onboarding '+(state.onboarding+1)+'/4':'8 online'}</span></div>
 ...
 ${state.survivors.length?`<div class="section-head compact-head"><h3>Survivor</h3><span>Tension ${Math.round(state.relations.tension)}</span></div><section class="card survivor-list survivor-dashboard compact-survivors">${state.survivors.map(s=>{const sq=survivorSideQuest(s);return `<button class="survivor" data-survivor="${s.id}">${avatarHtml(s)}<div class="grow"><b>${s.name}</b><small>${s.role}${sq&&!sq.done?` · ${sq.name}`:''}</small></div><div class="trust"><small>${trustLabelFor(s.trust).toUpperCase()}</small>${Math.round(s.trust)}</div></button>`}).join('')}</section>`:''}
 ...
 <div class="version">LOCKDOWN · PWA v0.19.4 · UI COHERENCE PASS</div>
 </main>`;
}
