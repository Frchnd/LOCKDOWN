...
function dashboard(){
 ...
 <section class="station-grid dashboard-stations">${stations.map(([id,n,ico,sub])=>{const locked=stationLocked(id)||isGameOver();const reason=stationLockReason(id);const upgradeKey=id==='security'?'keamanan':id;const hasLv=['generator','medis','kasur','security','workbench'].includes(id);return `<button class="station station-${id} ${locked?'locked':''} ${id===activeOnboarding?'active-objective':''}" data-panel="${id}" ${locked?'disabled':''}><div class="station-top"><span class="ico">${ico}</span>${hasLv&&!stationLocked(id)?`<span class="badge">LV ${state.upgrades[upgradeKey]}</span>`:''}</div><span class="station-copy"><b>${n}</b><small>${sub}</small></span>${reason?`<span class="lock-reason">${reason}</span>`:''}</button>`}).join('')}</section>
 ${state.survivors.length?`<div class="section-head"><h3>Survivor</h3><span>Tension ${Math.round(state.relations.tension)} · ${tensionLabel()}</span></div><section class="card survivor-list survivor-dashboard">${state.survivors.map(s=>{const sq=survivorSideQuest(s);return `<button class="survivor" data-survivor="${s.id}">${avatarHtml(s)}<div class="grow"><b>${s.name}</b><small>${s.role} · ${s.skill}</small>${sq&&!sq.done?`<em>SIDE // ${sq.name}</em>`:''}</div><div class="trust"><small>${trustLabelFor(s.trust).toUpperCase()}</small>${Math.round(s.trust)}</div></button>`}).join('')}</section>`:''}
 ...
 <div class="version">LOCKDOWN · PWA v0.19.3 · VISUAL ASSET INTEGRATION</div>
 </main>`;
}
...
function panel(){
 ...
 return `<main class="screen panel ${panelClass}"><header class="panel-head ${headClass}"><button class="back" data-act="back">←</button><div class="grow"><h1>${title}</h1>${subtitle?`<span>${subtitle}</span>`:''}</div><small>D${state.day} ${fmtHour()}</small></header>${panelVisual(p)}${body}</main>`;
}
